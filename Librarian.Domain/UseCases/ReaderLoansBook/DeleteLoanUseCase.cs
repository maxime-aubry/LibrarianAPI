...
namespace Librarian.Core.UseCases.ReaderLoansBook
{
    ...
    {
        public DeleteLoanUseCase(IReaderLoansBookRepository readerLoansBookRepository)
        {
        ...
        public async Task<bool> Handle(DeleteLoanRequest message, IOutputPort<UseCaseResponseMessage<string>> outputPort)
        {
            ...
            {
                GateawayResponse<string> response = await this.readerLoansBookRepository.Delete(message.Id);
